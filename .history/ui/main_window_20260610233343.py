...
def _patch_nav_button(btn):
    ...
    def patched_draw_icon(painter):
        """
        替换后的 _drawIcon 方法。
        与原始实现的区别仅为：绘制矩形从 QRectF(22,13,20,20) → QRectF(16,5,40,60)
        ...
        if btn._isSelectedTextVisible:            # 侧边栏展开时 → 文字可见
            rect = QRectF(16, 5, 40, 60)          # ┌── 可调整 ──┐
                                                  # (x=16, y=5, 宽=40, 高=60)
                                                  # x=16: 图标离左边缘 16px
                                                  # y=5:  图标离顶边缘 5px（贴顶）
                                                  # 宽40: 图标宽度（默认 20，放大约 2 倍）
                                                  # 高60: 图标高度
        else:                                     # 侧边栏折叠时 → 仅图标可见
            rect = QRectF(16, 5 + btn.iconAni.offset, 40, 60)
    ...
    def patched_draw_text(painter):
        ...
        # ┌── 可调整 ──┐
        rect = QRect(0, 38, btn.width(), 22)       # 文字矩形
                                                   # x=0:  离左边缘 0px（整行宽度）
                                                   # y=38: 离顶边缘 38px（原始为 32）
                                                   # 宽=按钮宽度（自动填满）
                                                   # 高=22: 文字行高（原始为 26）
